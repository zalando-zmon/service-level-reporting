...
from app.config import KAIROS_QUERY_LIMIT, KAIROSDB_URL
...
def query_sli(
    sli_name: str, sli_source: dict, start: int, end: Optional[int]
) -> Dict[datetime, float]:
    ...
    for result in data["queries"][0]["results"]:
        ...
        if not exclude:
            for ts, value in result["values"]:
                ...
                if aggregation_type == "weighted" and key_matches(
                    key, sli_source["aggregation"]["weight_keys"]
                ):
